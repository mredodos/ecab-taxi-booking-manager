(function ($) {
  ...
  function reset_type_select() {
    ...
    $.each(option, function (key, value) {
      $('.mpStyles .checkout select#type').append(
        $('<option></option>').attr('value', key).text(value)
      );
    });
    $('.mpStyles .checkout select#type').prop('disabled', false);
    type_rendering(
      $('.mpStyles .checkout select#type').find(':selected').val(),
      $('.mpStyles .checkout .open-modal').data('action')
    );
  }
  ...
  function prepare_text(action, field = null) {
    if (action === 'add') {
      ...
      $('.mpStyles .checkout input[name="placeholder"]').val(
  ...
  function prepare_select(action, field = null) {
    if (action === 'add') {
      ...
      let html = '';
      if (
        typeof field.attributes.options === 'object' &&
        !Array.isArray(field.attributes.options) &&
        field.attributes.options !== null
      ) {
        html += '<table>' + '<tbody class="ui-sortable">';
        for (const [key, value] of Object.entries(field.attributes.options)) {
          html +=
            ...
            '<div class="option-row">' +
            '<div class="input-cell">' +
            '<input type="text" name="option_value[]" placeholder="Option Value" value="' +
            key +
            '">' +
            '</div>' +
            '<div class="input-cell">' +
            '<input type="text" name="option_text[]" placeholder="Option Text" value="' +
            value +
            '">' +
            '</div>' +
            '<div class="action-cell">' +
  ...
  function prepare_header(action, field = null) {
    if (action === 'add') {
      // Show header type section, hide others
      $('.mpStyles .checkout .header-type-section').show();
      $('.mpStyles .checkout .custom-var-attr-section').hide();
      $('.mpStyles .checkout input[name="required"]').closest('label').hide();
      $('.mpStyles .checkout input[name="disabled"]').closest('label').hide();

      // Clear irrelevant fields
      $('.mpStyles .checkout input[name="placeholder"]').val('');
      $('.mpStyles .checkout input[name="validate"]').val('');
      $('.mpStyles .checkout input[name="class"]').val('');
      $('.mpStyles .checkout input[name="required"]').prop('checked', false);
      ...
      if (field && field.attributes && field.attributes.header_type) {
        $('.mpStyles .checkout select[name="header_type"]').val(
          field.attributes.header_type
        );
      }
  ...
  function prepare_other(action, field = null) {
    if (action === 'add') {
      // Hide header type section, show others
      $('.mpStyles .checkout .header-type-section').hide();
      $('.mpStyles .checkout .custom-var-attr-section').show();
      $('.mpStyles .checkout input[name="required"]').closest('label').show();
      $('.mpStyles .checkout input[name="disabled"]').closest('label').show();
  ...
  $(document).ready(function () {
    ...
    $('.mpStyles .checkout .open-modal').click(function () {
      ...
      $('.mpStyles .checkout #field-modal input[name="action"]').val(
        $(this).data('action')
      );
      $('.mpStyles .checkout #field-modal input[name="key"]').val(
        $(this).data('key')
      );
      if ($(this).data('action') == 'add') {
        ...
        $('.mpStyles .checkout input[name="required"]').prop(
          'checked',
          field.attributes.required == 1 ? true : false
        );
        $('.mpStyles .checkout input[name="disabled"]').prop(
          'checked',
          field.attributes.disabled == 1 ? true : false
        );
        type_rendering(
          $('.mpStyles .checkout input[name="new_type"]').val(),
          $('.mpStyles .checkout #field-modal input[name="action"]').val(),
    ...
    $('.mpStyles .checkout .close,.mpStyles .checkout .modal').click(
      function () {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (isChecked) {
            element
              .closest('tr')
              .find('td .checkout-disabled')
              .removeClass('dashicons dashicons-yes tips');
          } else {
            element
              .closest('tr')
              .find('td .checkout-disabled')
              .addClass('dashicons dashicons-yes tips');
          }
        },
      });
    });
