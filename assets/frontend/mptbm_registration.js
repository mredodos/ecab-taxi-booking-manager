...
function mptbm_set_cookie_distance_duration(start_place = '', end_place = '') {
  ...
  if (!mapContainer) {
    console.warn(
      'Map container #mptbm_map_area not found. Map initialization skipped.'
    );
    return false;
  }
...
function mptbm_map_area_init() {
  ...
  if (!mapContainer) {
    console.warn(
      'Map container #mptbm_map_area not found. Skipping map initialization.'
    );
    return false;
  }
  ...
  if (jQuery('#mptbm_map_start_place').length > 0) {
    ...
    let mptbm_restrict_search_to_country = jQuery(
      '[name="mptbm_restrict_search_country"]'
    ).val();
  ...
  if (
    ...
    jQuery('#mptbm_map_end_place').is(':visible')
  ) {
    let end_place = document.getElementById('mptbm_map_end_place');
...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    window.initializeGooglePlacesAutocomplete = function () {
      ...
      if (
        startPlaceInput &&
        !startPlaceInput.hasAttribute('data-autocomplete-initialized')
      ) {
        console.log(
          'Initializing Google Places autocomplete for pickup location'
        );
        var startPlaceAutocomplete = new google.maps.places.Autocomplete(
          startPlaceInput
        );
        var mptbmRestrictSearchToCountry = $(
          '[name="mptbm_restrict_search_country"]'
        ).val();
      ...
      if (
        endPlaceInput &&
        !endPlaceInput.hasAttribute('data-autocomplete-initialized') &&
        endPlaceInput.type !== 'hidden'
      ) {
        console.log(
          'Initializing Google Places autocomplete for dropoff location'
        );
        var endPlaceAutocomplete = new google.maps.places.Autocomplete(
          endPlaceInput
        );
        var restrictToCountry = $(
          '[name="mptbm_restrict_search_country"]'
        ).val();
    ...
    $(document).on('click', '.nextTab_prev, .nextTab_next', function () {
      setTimeout(function () {
        var nextButtonContainer = document.querySelector(
          '.get_details_next_link'
        );
        if (nextButtonContainer) {
          // Force a reflow to ensure proper positioning
          nextButtonContainer.style.display = 'none';
      ...
      $('.mptb-tabs li').each(function () {
        ...
        if (tabContent.length === 0) {
          ...
          if (tabContainerParent.length > 0) {
            var newTabContainer = $(
              '<div id="' + tabId + '" class="mptb-tab-content"></div>'
            );
    ...
    window.ensureLoadingGifExists = function () {
      ...
      if (loadingGif.length === 0 && tabContainer.length > 0) {
        ...
        var loadingGifHtml =
          '<div class="mptbm-hide-gif mptbm-gif" style="display: none;"><img src="' +
          window.location.origin +
          '/wp-content/plugins/ecab-taxi-booking-manager/assets/images/loader.gif" class="mptb-tabs-loader" /></div>';
        ...
        console.log(
          'Tab container not found, will create loading GIF when needed'
        );
        return false;
      }
      return true;
  ...
  $(document).on('click', '#mptbm_get_vehicle', function () {
    ...
    let mptbm_original_price_base = parent
      .find('[name="mptbm_original_price_base"]')
      .val();
    ...
    if (
      ...
      let selectedReturnTimeElement = parent
        .find('#mptbm_map_return_time')
        .closest('.mp_input_select')
        .find("li[data-value='" + return_time + "']");
    ...
    let selectedTimeElement = parent
      .find('#mptbm_map_start_time')
      .closest('.mp_input_select')
      .find("li[data-value='" + start_time + "']");
            ...
            if (!mptbm_enable_view_search_result_page) {
              ...
              $.ajax({
                ...
                data: {
                  ...
                  mptbm_max_passenger: parent
                    .find('#mptbm_max_passenger')
                    .val(),
  ...
  $(document).on('change', '#mptbm_map_start_date', function () {
    ...
    let mptbm_enable_return_in_different_date = $(
      '[name="mptbm_enable_return_in_different_date"]'
    ).val();
    let mptbm_buffer_end_minutes =
      parseInt($('[name="mptbm_buffer_end_minutes"]').val()) || 0;
    let mptbm_first_calendar_date = $(
      '[name="mptbm_first_calendar_date"]'
    ).val();
      ...
      if (selectedDate == dateValue) {
        ...
        $('.mp_input_select_list li').each(function () {
          ...
          if (timeValue > selectedTime) {
            $('#mptbm_map_return_time')
              .siblings('.mp_input_select_list')
              .append($(this).clone());
          }
        });
      } else {
          ...
          success: function (data) {
            ...
            target
              ...
              .done(function () {
                ...
                setTimeout(function () {
                  console.log(
                    'Select updated, options count:',
                    target.find('option:not([disabled])').length
                  );
                }, 100);
              });
          },
...
(function ($) {
  $(document).on('click', '.mp_quantity_minus, .mp_quantity_plus', function () {
    ...
    var transportPrice = parseFloat(
      $(`.mptbm_transport_select[data-post-id="${postId}"]`).attr(
      ...
      $summary
        ...
        .html(
          'x' +
            updatedVal +
            ' <span style="color:#000;">|&nbsp;&nbsp;</span>' +
            mp_price_format(transportPrice * updatedVal)
        );
    }
    ...
    $searchArea
      ...
      .attr('data-price', transportPrice * updatedVal);
  ...
  $(document).on(
    ...
    function () {
      ...
      let target_quantity_selector = parent.find(
        '.mptbm_quantity_selector_' + postId
      );

      // Toggle logic for quantity selector
      if (
        target_quantity_selector.length &&
        target_quantity_selector.hasClass('mptbm_booking_item_hidden')
      ) {
      ...
      if ($this.hasClass('active_select')) {
        ...
        parent
          ...
          .done(function () {
            ...
            let quantityInput = parent.find(
              `.mp_quantity_input[data-post-id="${post_id}"]`
            );
            let quantityVal = quantityInput.length
              ? parseInt(quantityInput.val()) || 1
              : 1;

            // Check if there's a custom message
            let customMessage = $this
              .closest('.mptbm_booking_item')
              .find('.mptbm-custom-price-message')
              .html();
            if (customMessage) {
              // If there's a custom message, show it with quantity
              target_summary
              ...
              target_summary
                ...
                .html(
                  'x' +
                    quantityVal +
                    ' <span style="color:#000;">|&nbsp;&nbsp;</span> ' +
                    mp_price_format(transport_price * quantityVal)
                );
            }
                  ...
                  success: function (data) {
                    target_extra_service_summary
                      ...
                      .done(function () {
                        ...
                        if (
                          target_extra_service.find(
                            '[name="mptbm_extra_service[]"]'
                          ).length > 0
                        ) {
                          target_summary.slideDown(400);
                        ...
                        if (
                          ...
                          parent
                            .find('.mptbm_book_now[type="button"]')
                            .trigger('click');
                        } else {
                          checkAndToggleBookNowButton(parent);
                        }
  ...
  function checkAndToggleBookNowButton(parent) {
    ...
    let extraServicesAvailable =
      parent.find('[name="mptbm_extra_service[]"]').length > 0;
  ...
  $(document).on('click', ".mptbm_book_now[type='button']", function () {
    ...
    let quantity =
      parseInt(
        parent.find(`.mp_quantity_input[data-post-id="${post_id}"]`).val()
      ) || 1;
    let mptbm_original_price_base = parent
      .find('[name="mptbm_original_price_base"]')
      .val();
  ...
  $(document).ready(function () {
    ...
    $('.mptb-tabs li').click(function () {
      ...
      var hasExistingContent =
        targetTabContainer.length > 0 &&
        targetTabContainer.html().trim() !== '';
      ...
      if (!hasExistingContent) {
        ...
        var loadingOverlay = $(
          '<div class="mptbm-loading-overlay" style="position: fixed !important; top: 50% !important; left: 50% !important; transform: translate(-50%, -50%) !important; z-index: 9999 !important; padding: 30px !important; text-align: center !important; margin-top: 40px !important;"><img src="' +
            window.location.origin +
            '/wp-content/plugins/ecab-taxi-booking-manager/assets/images/loader.gif" style="width: 80px !important; height: auto !important;" /></div>'
        );
        ...
        $.ajax({
          ...
          beforeSend: function () {
            ...
            if (tabContainer.length === 0) {
              ...
              if (tabContainerParent.length > 0) {
                var newTabContainer = $(
                  '<div id="' + tab_id + '" class="mptb-tab-content"></div>'
                );
                tabContainerParent.append(newTabContainer);
            ...
            if (tabContainer.length > 0) {
              tabContainer.html(
                '<div style="text-align: center; padding: 20px;"><p>Loading...</p><div style="margin-top: 10px;">Please wait while we load the booking form...</div></div>'
              );
            }
          },
          success: function (data) {
            ...
            setTimeout(function () {
              ...
              var currentTab = $('.mptb-tabs li.current').attr(
                'mptbm-data-tab'
              );
              var mapEnabled = $('.mptb-tabs li.current').attr(
                'mptbm-data-map'
              );
          ...
          error: function (response) {
            ...
            if (tabContainer.length > 0) {
              tabContainer.html(
                '<div style="text-align: center; padding: 20px; color: red;"><p>Error loading content. Please try again.</p></div>'
              );
            }
          },
        });
  ...
  $(document).on(
    'click',
    '#mptbm_manual_start_place, #mptbm_manual_end_place',
    function (e) {
      ...
      var $customWrapper = $(
        '<div class="mptbm-custom-select-wrapper" style="position: absolute !important; top: ' +
          (selectOffset.top + selectHeight + 2) +
          'px !important; left: ' +
          selectOffset.left +
          'px !important; width: ' +
          selectWidth +
          'px !important; z-index: 9999 !important; background: white !important; border: 1px solid #ddd !important; border-radius: 4px !important; box-shadow: 0 2px 8px rgba(0,0,0,0.1) !important;"></div>'
      );

      // Create search input
      var $searchInput = $(
        '<input type="text" class="mptbm-custom-search-input" placeholder="Search locations..." style="width: 100% !important; padding: 8px !important; border: none !important; border-bottom: 1px solid #eee !important; border-radius: 4px 4px 0 0 !important; font-size: 14px !important; box-sizing: border-box !important; background: #F5F6F8 !important; color: #222222 !important; font-weight: 400 !important; outline: none !important;" />'
      );

      // Create options container
      var $optionsContainer = $(
        '<div class="mptbm-custom-options" style="max-height: 200px !important; overflow-y: auto !important; background: white !important;"></div>'
      );
      ...
      $originalOptions.each(function () {
        ...
        optionsHtml +=
          '<div class="mptbm-custom-option ' +
          selectedClass +
          '" data-value="' +
          optionValue +
          '" style="padding: 8px !important; cursor: pointer !important; border-bottom: 1px solid #f5f5f5 !important; font-size: 14px !important; color: #222222 !important;">' +
          optionText +
          '</div>';
      });
      ...
      $(document).one('click', function (e) {
        if (
          !$(e.target).closest('.mptbm-custom-select-wrapper, #' + selectId)
            .length
        ) {
          $customWrapper.remove();
          // Restore map z-index
  ...
  $(document).on('click', '.mptbm-info-button', function (e) {
    ...
    var $content = $vehicleWrapper.find(
      '.mptbm-extra-info-content[data-post-id="' + postId + '"]'
    );
    ...
    $('.mptbm-info-button')
      .not($button)
      .css('background', 'var(--color_theme)')
      .find('i')
  ...
  $(document).on('click', function (e) {
    if (
      !$(e.target).closest('.mptbm-button-container, .mptbm-extra-info-content')
        .length
    ) {
...
function gm_authFailure() {
  ...
  jQuery('#mptbm_map_area').html(
    '<div class="mptbm-map-warning"><h6>' + warning + '</h6></div>'
  );
}
// Utility: Detect iOS
